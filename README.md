...
### 🎚️ 辨識靈敏度控制 (Sensitivity Control)
*   **即時靈敏度拉桿**：在掃描介面與系統設定中新增「靈敏度控制」功能，讓使用者能根據環境光線與背景複雜度，動態調整 YOLO 辨識的信心門檻 (Confidence Threshold)。
*   **品質精準化**：提供從「穩定精準 (Strict)」到「靈活偵測 (Sensitive)」的無縫切換，適應多變的廚房光影環境。
*   **UI/UX 優化**：同步更新至個人設定與相機掃描頁面，操作無死角。
